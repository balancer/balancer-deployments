...
import { FP_ONE, fp, fromFp } from '@helpers/numbers';
...
describeForkTest('GaugeWorkingBalanceHelper-L2', 'polygon', 42002545, function () {
  ...
  describe('with veBAL', () => {
    before('setup contracts', async () => {
      ...
      veDelegationProxy = await task.deploy(
        'VotingEscrowDelegationProxy',
        [vault.address, votingEscrow.address, ZERO_ADDRESS],
        veBALHolder,
        forceDeploy
      );
      workingBalanceHelper = await task.deploy(
        'GaugeWorkingBalanceHelper',
        [veDelegationProxy.address, false],
        veBALHolder,
        forceDeploy
      );
    });
