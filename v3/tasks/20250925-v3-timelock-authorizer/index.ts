...
export default async (task: Task, { force, from }: TaskRunOptions = {}): Promise<void> => {
  ...
  const authorizerArgs = [migrator.address, input.Root, input.Vault, input.RootTransferDelay];

  await task.verify('TimelockAuthorizer', authorizer.target as string, authorizerArgs);
  ...
  await task.verify('TimelockExecutionHelper', executor.target as string, []);
};
