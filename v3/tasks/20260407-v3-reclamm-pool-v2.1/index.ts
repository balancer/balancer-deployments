...
export default async (task: Task, { force, from }: TaskRunOptions = {}): Promise<void> => {
  ...
  const helper = await task.deployAndVerify('ReClammPoolHelper', [input.Vault], from, force);

  const args = [input.Vault, helper.target, input.PauseWindowDuration, input.FactoryVersion, input.PoolVersion];
  const factory = await task.deployAndVerify('ReClammPoolFactory', args, from, force);

  // ReClamm parameters.
  const DAILY_PRICE_SHIFT_EXPONENT = fp(0.01); // 1%
  ...
  if (task.mode === TaskMode.LIVE) {
    ...
    await task.verify('ReClammPool', mockPool.target.toString(), [poolParams, input.Vault, helper.target]);
  }
};
