...
describeForkTest.skip('BalancerContractRegistryInitializer', 'mainnet', 21862412, function () {
  ...
  before('grant permissions', async () => {
    ...
    await authorizer.connect(govMultisig).grantRole(await authorizer.DEFAULT_ADMIN_ROLE(), registryInitializer.target as string);
  });
  ...
  it('renounces the admin role', async () => {
    expect(await authorizer.hasRole(await authorizer.DEFAULT_ADMIN_ROLE(), registryInitializer.target as string)).to.be.false;
  });
  ...
  it('has registered the pool factories', async () => {
    ...
    info = await registry.getBalancerContractInfo(lbpFactory.target as string);
    _validateInfo(info);
  });
