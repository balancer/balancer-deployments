...
export default async (task: Task, { force, from }: TaskRunOptions = {}): Promise<void> => {
  ...
  const factoryArgs = [input.StableSurgeHook, input.PauseWindowDuration, input.FactoryVersion, input.PoolVersion];
  const factory = await task.deployAndVerify('StableSurgePoolFactory', factoryArgs, from, force);
