...
describeForkTest.only('Vault Explorer', 'mainnet', 21336938, function () {
  let task: Task;
  let vault: Contract;
  ...
  it('checks pool tokens', async () => {
    ...
    const poolTokens = (await extensionEntrypoint.getPoolTokens(mockPool.target.toString())).map((token: string) =>
      token.toLowerCase()
    );
