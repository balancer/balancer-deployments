 ...
 */
export function toNormalizedWeights(weights: BigNumberish[]): bigint[] {
  ...
  const normalizedInput = weights.map((weight) => bn(weight));
  const sum = normalizedInput.reduce((total, weight) => total + weight, 0n);
  if (sum === ONE) return normalizedInput;

  const normalizedWeights: bigint[] = [];
  let normalizedSum = 0n;
  for (let index = 0; index < normalizedInput.length; index++) {
    if (index < weights.length - 1) {
      normalizedWeights[index] = (normalizedInput[index] * ONE) / sum;
      normalizedSum += normalizedWeights[index];
    } else {
...
export const isNormalizedWeights = (weights: BigNumberish[]): boolean => {
  const totalWeight = weights.reduce((total: bigint, weight) => total + bn(weight), 0n);
