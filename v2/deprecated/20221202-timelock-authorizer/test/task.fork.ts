...
describeForkTest.skip('TimelockAuthorizer', 'mainnet', 16076200, function () {
  ...
  before('load old authorizer and impersonate multisig', async () => {
    ...
    await oldAuthorizer.connect(multisig).grantRolesToMany([setAuthorizerActionId], [migrator.target.toString()]);
  });
  ...
  it('does not set the new authorizer immediately', async () => {
    expect(await newAuthorizer.isRoot(migrator.target.toString())).to.be.true;
    expect(await vault.getAuthorizer()).to.be.equal(oldAuthorizer.target.toString());
  });
  ...
  it('allows minting after the migration', async () => {
    ...
    const tx = await balancerTokenAdmin.connect(balancerMinterSigner).mint(balancerMinter.target.toString(), 100);

    expectEvent.inIndirectReceipt(await tx.wait(), balancerToken.interface, 'Transfer', {
      from: ZERO_ADDRESS,
      to: balancerMinter.target.toString(),
      value: 100,
    });
  });
