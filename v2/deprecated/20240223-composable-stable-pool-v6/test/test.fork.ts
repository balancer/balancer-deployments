...
describeForkTest.skip('ComposableStablePool V6', 'mainnet', 19292000, function () {
  ...
  let dai: Contract, usdc: Contract, aura: Contract, graviAura: Contract;
  ...
  const AURA_HOLDER = '0xfc78f8e1Af80A3bF5A1783BB59eD2d1b10f78cA9';
  const GRAVI_AURA_HOLDER = '0x08E29a38e6F469834dAB09142dA41694C15d63CD';
  ...
  let factoryDeploymentTimestamp: bigint;
  ...
  before('run task', async () => {
    task = new Task('20240223-composable-stable-pool-v6', TaskMode.TEST, getForkedNetwork(hre));
    await task.run({ force: true });
    factory = await task.deployedInstance('ComposableStablePoolFactory');
    factoryDeploymentTimestamp = await currentTimestamp();
  });
  ...
  describe('read-only reentrancy protection', () => {
    ...
    context('when the target pool is protected', () => {
      sharedBeforeEach('deploy pool with rate providers', async () => {
        ...
        pool = await createPool(tokens, rateProvider.target.toString());
        poolId = await pool.getPoolId();
      });
  ...
  describe('pause window', () => {
    ...
    sharedBeforeEach(async () => {
      ...
      expect(await currentTimestamp()).to.be.lt(factoryDeploymentTimestamp + TOLERANCE);
    });
