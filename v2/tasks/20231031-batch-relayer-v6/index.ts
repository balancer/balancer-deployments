...
export default async (task: Task, { force, from }: TaskRunOptions = {}): Promise<void> => {
  ...
  const relayerAddress = relayer.target as string;

  const relayerArgs = [input.Vault, relayerLibrary.target as string, queryLibrary, input.Version]; // See BalancerRelayer's constructor
