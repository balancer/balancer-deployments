...
describeForkTest.only('OmniVotingEscrow', 'mainnet', 17331260, function () {
  ...
  context('setup and transfer ownership', async () => {
    it('wires trusted endpoint', async () => {
      ...
      await (omniVotingEscrow.connect(deployer) as Contract).setTrustedRemoteAddress(
        POLYGON_LZ_CHAIN_ID,
        encodedEndpoint
      );
    ...
    it('transfer ownership to LM multisig', async () => {
      ...
      await (omniVotingEscrow.connect(deployer) as Contract).transferOwnership(LM_MULTISIG);
