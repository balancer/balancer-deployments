...
describeForkTest.skip('OptimismRootGaugeFactoryV2', 'mainnet', 15397200, function () {
  ...
  it('add gauge to gauge controller', async () => {
    await (gaugeAdder.connect(admin) as Contract).addGaugeFactory(factory.target as string, 5); // Optimism is Gauge Type 5
    await (gaugeAdder.connect(admin) as Contract).addOptimismGauge(gauge.target as string);
